...
def create_comparison_charts(avg_scores, prompt_keys, output_cols):
    """
    Create both bar chart and spider chart for comparison visualization
    """
    import plotly.graph_objects as go
    ...
    if not has_scores:
        ...
        return fig, None  # Return none for spider chart
    
    # 1. Create the bar chart (original chart)
    bar_fig = go.Figure()
    ...
    try:
        # For spider chart we need data in a different format
        for i, output_col in enumerate(output_cols):
            # Get the scores for this output column
            scores = []
            for prompt_key in prompt_keys:
                if output_col in avg_scores and prompt_key in avg_scores[output_col]:
                    score = avg_scores[output_col][prompt_key]
            ...
            # Add a trace for this output column
        ...
        # Update layout for the spider chart
        spider_fig.update_layout(
            title="Radar Chart of Metrics by Output Column",
            polar=dict(
                radialaxis=dict(
                    visible=True,
                    range=[0, max(5, max([max(avg_scores.get(col, {}).values()) 
                                         for col in output_cols 
                                         if col in avg_scores and avg_scores[col].values()]) + 0.5)]
                )
            ),
            showlegend=True,
            height=500,
        ...
        spider_fig = go.Figure()
        spider_fig.update_layout(
            title="Error creating spider chart",
            annotations=[dict(
                text=f"An error occurred: {str(e)}",
                showarrow=False,
                xref="paper",
...
def display_evaluation_results_multi_output(results, prompt_keys, output_cols):
    ...
    bar_fig, spider_fig = create_comparison_charts(avg_scores, prompt_keys, output_cols)
    ...
    if len(output_cols) >= 2:
        ...
        with tab1:
            ...
            # Add spider chart if it was created successfully
            if spider_fig is not None:
...
def display_evaluation_results_multi_output(results, prompt_keys, output_cols):
    ...
    if len(output_cols) >= 2:
        ...
        with tab1:
            st.subheader("Comparison of Average Scores")
            fig = create_comparison_charts(avg_scores, prompt_keys, output_cols)
