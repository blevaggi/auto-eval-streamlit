...
ddef create_spider_chart(avg_scores, prompt_keys, output_cols):
    """
    Create a proper radar chart that shows all metrics
    """
    import plotly.graph_objects as go
    import pandas as pd
    
    # First, let's print what we're working with to diagnose issues
    st.write("Debugging Spider Chart Data:")
    st.write("Output Columns:", output_cols)
    st.write("Prompt Keys (metrics):", prompt_keys)
    
    # Create a readable version of the data
    display_data = {}
    for output_col in output_cols:
        if output_col in avg_scores:
            display_data[output_col] = {}
            for prompt_key in prompt_keys:
                if prompt_key in avg_scores[output_col]:
                    display_data[output_col][prompt_key] = avg_scores[output_col][prompt_key]
                else:
                    display_data[output_col][prompt_key] = 0
    
    st.write("Average Scores for Chart:", display_data)
    
    # Create a radar chart with explicit configuration
    fig = go.Figure()
    
    # Extract the metric names to make them more readable
    # Convert from 'metricName::paramName' to just 'paramName'
    shortened_metrics = []
    for key in prompt_keys:
        if "::" in key:
            shortened_metrics.append(key.split("::")[-1])
        else:
            shortened_metrics.append(key)
    
    # Add each output column as a trace
    for output_col in output_cols:
        # Collect values for this output column
        values = []
        for prompt_key in prompt_keys:
            if output_col in avg_scores and prompt_key in avg_scores[output_col]:
                score = avg_scores[output_col][prompt_key]
        ...
        # Only add the trace if we have values
        if values and any(v > 0 for v in values):
            fig.add_trace(go.Scatterpolar(
                r=values,
                theta=shortened_metrics,  # Use shortened metric names
                fill='toself',
                name=output_col
            ))
    
    # If we didn't add any traces, add a dummy one to avoid errors
    if len(fig.data) == 0:
        fig.add_trace(go.Scatterpolar(
            r=[0] * len(shortened_metrics),
    ...
    max_value = 1  # Default minimum
    for output_col in output_cols:
        if output_col in avg_scores:
            for prompt_key in prompt_keys:
                if prompt_key in avg_scores[output_col]:
                    val = avg_scores[output_col][prompt_key]
                    if val is not None and val > max_value:
                        max_value = val
    
    # Add some padding to the maximum
