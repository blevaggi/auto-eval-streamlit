...
def create_spider_chart(avg_scores, prompt_keys, output_cols):
    """
    Create a simple, reliable spider/radar chart for metric comparison with debug info
    """
    import plotly.graph_objects as go
    import pandas as pd
    
    # Print debug info to help diagnose issues
    st.write("Debug - Creating Spider Chart")
    st.write("Number of prompt keys:", len(prompt_keys))
    st.write("Output columns:", output_cols)
    
    # Convert to a dataframe for easier handling and debugging
    data = []
    for output_col in output_cols:
    ...
    # Create a dataframe and show it for debugging
    df = pd.DataFrame(data)
    st.write("Data for spider chart:")
    st.dataframe(df)
    
    # Create a super simple radar chart that should work reliably
    fig = go.Figure()
    ...
    for i, row in df.iterrows():
        ...
        # Debug the values for this trace
        st.write(f"Values for {output_name}:", values)
