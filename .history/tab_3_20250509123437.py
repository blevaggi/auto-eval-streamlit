...
def create_spider_chart(avg_scores, prompt_keys, output_cols):
    """
    Create a spider/radar chart for metric comparison
    """
    import plotly.graph_objects as go
    ...
    if not has_scores:
        ...
        spider_fig.update_layout(
            title="No scores available for radar visualization",
            annotations=[dict(
                text="No numeric scores were found in the evaluation results.",
                showarrow=False,
                xref="paper",
                yref="paper",
                x=0.5,
    ...
    try:
        # Find the maximum score for scaling the chart
        max_score = 0
        for output_col in output_cols:
            if output_col in avg_scores:
                for prompt_key in prompt_keys:
        ...
        # Default to 5 if we don't have a reasonable max
        if max_score <= 0:
            max_score = 5
        ...
        for output_col in output_cols:
            ...
            # Must close the loop by repeating the first score at the end
            if len(scores) > 0:
                scores.append(scores[0])
                metrics = prompt_keys + [prompt_keys[0]]
        ...
        spider_fig = go.Figure()
        spider_fig.update_layout(
            title="Error creating radar chart",
            annotations=[dict(
                text=f"An error occurred: {str(e)}",
                showarrow=False,
                xref="paper",
...
def display_evaluation_results_multi_output(results, prompt_keys, output_cols):
    ...
    if len(output_cols) >= 2:
        ...
        with tab1:
            ...
            # Bar chart
            bar_fig = create_comparison_chart(avg_scores, prompt_keys, output_cols)
