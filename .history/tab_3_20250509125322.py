...
def create_spider_chart(avg_scores, prompt_keys, output_cols):
    """
    Create a radar chart with multiple traces - one for each output column
    """
    import plotly.graph_objects as go
    import pandas as pd
    
    # Debug output to see what's going on
    st.write("Raw avg_scores structure:", avg_scores)
    ...
    st.write("DataFrame for chart:", df)
    ...
    for output_col in df.columns:
        ...
        # Debug this trace
        st.write(f"Adding trace for {output_col} with values: {values}")
    ...
    fig.update_layout(
        ...
        height=500,
        width=700,
        title="Metric Comparison by Output Column"
    )
