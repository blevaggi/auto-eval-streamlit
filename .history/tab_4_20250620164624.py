...
# Add this new function to replace the existing batch processing functions

def run_single_pairwise_evaluation_threadsafe(client, model, eval_prompt, data_to_evaluate, row_index, prompt_key, is_reversed=False):
    """
    Run a single pairwise evaluation in a thread-safe manner
    Returns a tuple of (row_index, prompt_key, result, is_reversed)
    """
    try:
        # Add explicit instruction to return JSON to the prompt
        eval_prompt_with_json = eval_prompt + "\n\nPlease provide your response in JSON format."
        ...
        return (row_index, prompt_key, response.choices[0].message.content, is_reversed)
    except Exception as e:
        return (row_index, prompt_key, {"error": str(e), "status": "failed"}, is_reversed)


def run_parallel_pairwise_evaluations(client, evaluation_model, pairwise_prompts, data_rows, 
                                     input_col, output_a_col, output_b_col, is_conversation=False,
                                     reduce_order_bias=False, batch_size=5, max_workers=5):
    """
    Run pairwise evaluations in parallel batches with automatic saving between batches
    Based on the Tab 3 parallel processing approach
    ...
    Args:
        ...
        pairwise_prompts: Dictionary of prompt templates
        data_rows: List of data rows to evaluate
        input_col: Column name for input (None for conversations)
        output_a_col: Column name for output A
        output_b_col: Column name for output B
        is_conversation: Whether this is conversation evaluation
        reduce_order_bias: Whether to run A vs B and B vs A
    ...
    if "batch_pairwise_results" not in st.session_state:
        st.session_state.batch_pairwise_results = {}
        st.session_state.batch_pairwise_progress = 0
        
        # Calculate total evaluations
        evals_per_row = len(pairwise_prompts)
        if reduce_order_bias:
            evals_per_row *= 2  # Double for A/B and B/A comparisons
        st.session_state.batch_pairwise_total = len(data_rows) * evals_per_row
    ...
    completed_batches = st.session_state.get("completed_pairwise_batch_count", 0)
    ...
    try:
        ...
        for batch_idx in range(completed_batches, total_batches):
            ...
            for row_idx, row in enumerate(current_batch):
                ...
                output_a_text = str(row[output_a_col]) if output_a_col and row[output_a_col] is not None else ""
                output_b_text = str(row[output_b_col]) if output_b_col and row[output_b_col] is not None else ""
                
                # Skip if missing required data
                if (not is_conversation and (not input_text.strip() or not output_a_text.strip() or not output_b_text.strip())) or \
                   (is_conversation and (not output_a_text.strip() or not output_b_text.strip())):
                    continue
                
                # Process each pairwise prompt
                for prompt_key, prompt_template in pairwise_prompts.items():
                    # First evaluation: A vs B (normal order)
                    if is_conversation:
                        formatted_prompt = prompt_template.replace("{conversation_a}", output_a_text).replace("{conversation_b}", output_b_text)
                    else:
                        formatted_prompt = prompt_template.replace("{input}", input_text).replace("{output_a}", output_a_text).replace("{output_b}", output_b_text)
                    
                    # Create task tuple for A vs B
                    task = (
                        client, evaluation_model, formatted_prompt, 
                        {"input": input_text, "output_a": output_a_text, "output_b": output_b_text},
                        global_row_idx, prompt_key, False  # is_reversed = False
                    )
                    evaluation_tasks.append(task)
                    
                    # If reducing order bias, add B vs A evaluation
                    if reduce_order_bias:
                        if is_conversation:
                            formatted_prompt_reversed = prompt_template.replace("{conversation_a}", output_b_text).replace("{conversation_b}", output_a_text)
                        else:
                        ...
                        # Create task tuple for B vs A
                        task_reversed = (
                            client, evaluation_model, formatted_prompt_reversed, 
                            {"input": input_text, "output_a": output_b_text, "output_b": output_a_text},
                            global_row_idx, prompt_key, True  # is_reversed = True
                        )
                        evaluation_tasks.append(task_reversed)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                # Submit all tasks
                future_to_task = {
                    executor.submit(run_single_pairwise_evaluation_threadsafe, *task): task 
                ...
                for future in concurrent.futures.as_completed(future_to_task):
                    try:
                        row_idx, prompt_key, result, is_reversed = future.result()
                        
                        # Initialize row result if not exists
                        if row_idx not in batch_results:
                            batch_results[row_idx] = {"row_index": row_idx, "evals": {}}
                        
                        # Store result based on whether we're reducing order bias
                        if reduce_order_bias:
                            # Initialize the prompt key structure if needed
                            if prompt_key not in batch_results[row_idx]["evals"]:
                                batch_results[row_idx]["evals"][prompt_key] = {}
                            
                            # Store A vs B or B vs A result
                            if is_reversed:
                                batch_results[row_idx]["evals"][prompt_key]["ba_result"] = result
                            else:
                                batch_results[row_idx]["evals"][prompt_key]["ab_result"] = result
                            
                            # If we have both results, combine them
                            eval_data = batch_results[row_idx]["evals"][prompt_key]
                            if "ab_result" in eval_data and "ba_result" in eval_data:
                                combined_result = combine_pairwise_results(eval_data["ab_result"], eval_data["ba_result"])
                                batch_results[row_idx]["evals"][prompt_key]["combined_result"] = combined_result
                        else:
                            # Single evaluation - store directly
            ...
            for row_idx, row_result in batch_results.items():
                st.session_state.batch_pairwise_results[row_idx] = row_result
            
            # Update progress tracking
            st.session_state.completed_pairwise_batch_count = batch_idx + 1
            st.session_state.batch_pairwise_progress += len(batch_results)
            ...
            batch_summary_placeholder.success(
                ...
                f"Total progress: {len(st.session_state.batch_pairwise_results)}/{total_rows} rows"
            )
            ...
            if batch_idx < total_batches - 1:
                with st.expander(f"Intermediate Results (after batch {batch_idx + 1})", expanded=False):
                    show_intermediate_pairwise_results(st.session_state.batch_pairwise_results, pairwise_prompts, reduce_order_bias)
        
        # Mark evaluation as complete
        st.session_state.pairwise_evaluation_complete = True
        
        # Clear batch tracking since we're done
        st.session_state.completed_pairwise_batch_count = 0
        
        # Clear placeholders
        status_placeholder.success(f"🎉 All pairwise evaluations complete! Processed {len(st.session_state.batch_pairwise_results)} rows")
        ...
        final_results = []
        for row_idx in sorted(st.session_state.batch_pairwise_results.keys()):
            row_result = st.session_state.batch_pairwise_results[row_idx]
        ...
        st.error(f"Evaluation interrupted. Progress saved: {len(st.session_state.batch_pairwise_results)} rows completed.")
        raise


def show_intermediate_pairwise_results(batch_results, pairwise_prompts, reduce_order_bias):
    """
    Show a summary of intermediate pairwise results without complex visualizations
    """
    ...
    # Count results by category for summary
    a_wins = 0
    b_wins = 0
    tied = 0
    
    for row_data in batch_results.values():
        for eval_key, eval_result in row_data.get("evals", {}).items():
            # Extract winner based on format
            winner = "Unknown"
            try:
                if reduce_order_bias and isinstance(eval_result, dict) and "combined_result" in eval_result:
                    combined = eval_result["combined_result"]
                    if isinstance(combined, dict) and "final_result" in combined:
                        result_text = combined["final_result"].lower()
                        if "a win" in result_text:
                    ...
                    eval_data = json.loads(eval_result)
                    winner_text = str(eval_data.get("winner", eval_data.get("preference", eval_data.get("result", "")))).lower()
                    if "a" in winner_text and ("better" in winner_text or "win" in winner_text):
                        winner = "A"
                    elif "b" in winner_text and ("better" in winner_text or "win" in winner_text):
                        winner = "B"
                    elif "tie" in winner_text or "equivalent" in winner_text:
                        winner = "Tie"
            except:
                pass
    ...
    col1.metric("A Wins", a_wins)
    col2.metric("B Wins", b_wins)
    col3.metric("Ties", tied)


def add_tab4_content_parallel():
    """
    Enhanced Tab 4 with parallel processing based on Tab 3 approach
    """
    st.header("Pairwise Evaluations (Parallel Processing)")
    st.info("Upload a dataset to compare paired outputs using the generated metrics. Processes evaluations in parallel with automatic saving between batches.")
    ...
    has_in_progress = "batch_pairwise_results" in st.session_state and st.session_state.batch_pairwise_results
    completed_batches = st.session_state.get("completed_pairwise_batch_count", 0)
    
    if has_in_progress:
        st.warning(f"📊 **Pairwise evaluation in progress**: {len(st.session_state.batch_pairwise_results)} rows completed, {completed_batches} batches finished.")
        
        col1, col2, col3 = st.columns(3)
        with col1:
            if st.button("▶️ Resume Evaluation", type="primary", key="resume_pairwise"):
                # Will be handled in the main processing section
                st.rerun()
        with col2:
            if st.button("👀 View Current Results", key="view_pairwise"):
                show_intermediate_pairwise_results(st.session_state.batch_pairwise_results, pairwise_prompts, False)
        with col3:
            if st.button("🗑️ Reset & Start Over", key="reset_pairwise"):
                # Clear all evaluation state
                for key in list(st.session_state.keys()):
                    if key.startswith("batch_pairwise"):
                        del st.session_state[key]
                if "completed_pairwise_batch_count" in st.session_state:
                    del st.session_state.completed_pairwise_batch_count
                st.rerun()
    ...
    uploaded_file = st.file_uploader(
        "Upload a CSV, XLSX, or XLS file with paired outputs", 
        type=["csv", "xlsx", "xls"],
        key="parallel_pairwise_file_uploader"
    )
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            if is_conversation:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    conversation_a_col = st.selectbox("Conversation A Column", [""] + column_names, key="parallel_conv_a")
                with col2:
                    conversation_b_col = st.selectbox("Conversation B Column", [""] + column_names, key="parallel_conv_b")
                input_col = None
                output_a_col = conversation_a_col
                output_b_col = conversation_b_col
            else:
                ...
                col1, col2, col3 = st.columns(3)
                with col1:
                    input_col = st.selectbox("Input Column", [""] + column_names, key="parallel_input")
                with col2:
                    output_a_col = st.selectbox("Output A Column", [""] + column_names, key="parallel_output_a")
                with col3:
                    output_b_col = st.selectbox("Output B Column", [""] + column_names, key="parallel_output_b")
                        ...
                        if isinstance(param, dict) and len(param) > 0:
                            ...
                            if prompt_key in pairwise_templates:
                                is_selected = st.checkbox(
                                    f"{param_key}: {param_description}", 
                                    value=True,
                                    key=f"parallel_pairwise_{prompt_key}"
                                )
                                if is_selected:
                                    selected_metrics[prompt_key] = pairwise_templates[prompt_key]
            ...
            # Validation
            if is_conversation and (not output_a_col or not output_b_col):
                st.warning("Please select both conversation columns.")
            elif not is_conversation and (not input_col or not output_a_col or not output_b_col):
                st.warning("Please select input, output A, and output B columns.")
            elif not selected_metrics:
                ...
                # Run evaluations button
                button_text = "🚀 Start Parallel Pairwise Evaluation"
                if has_in_progress:
                    button_text = "▶️ Continue Evaluation"
                
                if st.button(button_text, use_container_width=True, type="primary", key="run_parallel_pairwise"):
                    # Check if we have an API client
                    if not st.session_state.client:
                        st.error("Please set up your API key in the sidebar first.")
                        return
                    ...
                    try:
                        st.markdown("---")
                        st.subheader("🚀 Parallel Pairwise Evaluation Progress")
                        
                        results = run_parallel_pairwise_evaluations(
                            client=client,
                            evaluation_model=eval_model,
                            pairwise_prompts=selected_metrics,
                            data_rows=data_to_evaluate,
                        ...
                        st.success(f"🎉 All pairwise evaluations completed successfully!")
                        ...
                        st.markdown("---")
                        st.subheader("📊 Complete Pairwise Evaluation Results")
                        ...
                        if "batch_pairwise_results" in st.session_state and st.session_state.batch_pairwise_results:
                            with st.expander("View Partial Results", expanded=False):
                                show_intermediate_pairwise_results(st.session_state.batch_pairwise_results, selected_metrics, reduce_order_bias)
    ...
    # Show previous results if available
        ...
        # Get the saved settings from session state
        reduce_order_bias = st.session_state.get("pairwise_reduce_order_bias", False)
        selected_metrics = st.session_state.get("pairwise_selected_metrics", list(pairwise_templates.keys()))
