...
def display_evaluation_results_multi_output(results, prompt_keys, output_cols):
    ...
    if len(output_cols) > 2:
        # Create tabs with comparison first
        tab_names = ["Comparison"] + [f"Results: {col}" for col in output_cols]
    else:
        # Original order for 1-2 columns
    ...
    # Keep track of which tab we're on
    tab_index = 0
    
    # Display comparison visualization in the comparison tab
    # For >2 columns, this is the first tab (index 0)
    # For <=2 columns, this is the last tab (after all result tabs)
    if len(output_cols) > 2:
        with tabs[0]:
            st.subheader("Comparison of Average Scores")
            fig = create_comparison_chart(avg_scores, prompt_keys, output_cols)
            st.plotly_chart(fig, use_container_width=True)
        tab_index = 1  # Next tab will be at index 1
    ...
    # For <=2 columns, display comparison in the last tab
    if len(output_cols) <= 2:
        with tabs[-1]:
            st.subheader("Comparison of Average Scores")
            fig = create_comparison_chart(avg_scores, prompt_keys, output_cols)
...
def export_evaluation_results_multi_output(results, filename_prefix="evaluation_results"):
    ...
    with col1:
        # Use a unique key with both timestamp and uuid for the download button
        st.download_button(
            label="💾 Download Results as JSON",
            data=json_data,
            file_name=f"{filename_prefix}_{timestamp}.json",
            mime="application/json",
            key=f"json_download_{timestamp}_{unique_id}"  # More unique key
        )
    ...
    try:
        ...
        with col2:
            ...
            st.download_button(
                ...
                key=f"excel_download_{timestamp}_{unique_id}"  # More unique key
            )
    except Exception as e:
...
def add_tab3_content():
    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            if is_conversation and not output_cols:
                ...
                run_button_key = "run_evaluations_button_" + str(hash(str(selected_metrics) + str(output_cols)))
                if st.button("Run Evaluations", key=run_button_key):
                    ...
                    try:
                        ...
                        st.session_state.last_evaluation_prompt_keys = list(selected_metrics.keys())
                        st.session_state.last_evaluation_output_cols = list(multi_results.keys())
    ...
    # Check if we have results to show
        ...
        prompt_keys = st.session_state.get("last_evaluation_prompt_keys", [])
        if not prompt_keys and "pipeline_results" in st.session_state:
            # Fallback to all parameter templates
            prompt_keys = list(st.session_state.pipeline_results.get("parameter_prompt_templates", {}).keys())
        
        output_cols = st.session_state.get("last_evaluation_output_cols", [])
        if not output_cols:
            # Fallback to keys in results
            output_cols = list(st.session_state.multi_evaluation_results.keys())
        ...
        # Add export options with a unique session ID to prevent state issues
        import uuid
        session_id = st.session_state.get("export_session_id", str(uuid.uuid4()))
        st.session_state.export_session_id = session_id
        
        st.subheader("Export Results")
        export_evaluation_results_multi_output(st.session_state.multi_evaluation_results, f"evaluation_results_{session_id}")